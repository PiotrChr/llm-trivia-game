...
def process_single_attempt(batch, attempt, delay):
    try:
        result, finish_reason = add_hints_and_multiple_choice(batch)
...
def read_output_file(output_file):
    try:
        ...
        print('Output file not found. Creating new file: ', output_file)
...
def process_questions_in_batches(input_file, output_file, failed_batch_file, content_filter_file, batch_size=40):
    try:
        ...
        questions = read_output_file(input_file)
        ...
        processed_ids = {q['question'] for q in processed_questions if 'question' in q}

        questions_to_process = [q for q in questions if 'question' in q and q['question'] not in processed_ids and q['question'] not in failed_ids and q['question'] not in content_filtered_ids]
